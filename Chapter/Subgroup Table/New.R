...
for (i in 1:10001){
  ...
  accuracy <- myfunction(New, qure_ai_score[i])
...
for (i in 1:101){
  ...
  accuracy_both <- myfunction(New, CAD6[i])
...
for (i in 1:10001){
  ...
  accuracy <- myfunction(New, LunitVector[i])
  mylist[[i]] <- list(accuracy)
...
for (i in 1:10001){
  ...
  accuracy <- myfunction(New, JF1Vector[i])
  mylist[[i]] <- list(accuracy)
...
for (i in 1:10001){
  ...
  accuracy <- myfunction(New, JF2Vector[i])
  mylist[[i]] <- list(accuracy)
...
for (i in 1:10001){
  ...
  accuracy <- myfunction(New, IF1Vector[i])
  mylist[[i]] <- list(accuracy)
...
for (i in 1:10001){
  ...
  accuracy <- myfunction(New, IF2Vector[i])
  mylist[[i]] <- list(accuracy)
...
for (i in 1:10001){
  ...
  accuracy <- myfunction(New, IF3Vector[i])
  mylist[[i]] <- list(accuracy)
...
CAD_Xpert_plot_New <- CAD_Xpert[, c(16, 21, 14, 13, 15, 1:3, 4,7, 5,6,8:12, 23:27, 22)]

CAD_Xpert_plot_New$subgroup <- "New"

# CAD_Xpert_select <- CAD_Xpert_select[!CAD_Xpert_select$DeepLearningSystem %in% c("IF1", "JF2", "IF3"), ]

# write.csv(CAD_Xpert_plot_New, "Results/CAD_Xpert Cutoffs TABLE_New.csv")
# write.csv(CAD_Xpert_select, "Results/Supp Tab_New.csv", row.names = F)
