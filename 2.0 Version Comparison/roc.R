...
ggROC_qure <- base + theme_light() + geom_abline(slope=1, intercept = 0, linetype = "dashed", alpha=0.7, color = "grey") + coord_equal() + annotate("text", x = .75, y = .15, label = paste("qXRv2: ", round(roc_qure2[2],4), " (95% CI:", round(roc_qure2[1],4), "-", round(roc_qure2[3],4), ")", ", ", "\n",
                                                   "qXRv3: ", round(roc_qure3[2],4), " (95% CI:", round(roc_qure3[1],4), "-", round(roc_qure3[3],4), ")", 
                                                   sep = ""), size = 3) + 
labs(x = "1-Specificity", y= "Sensitivity", subtitle = paste0("The ROC curves of qXR using Xpert results as the reference (n=", length(MDF$PID_OMRS), ")"))+theme(legend.position = "bottom")
...
ggROC_CAD <- base + theme_light() + geom_abline(slope=1, intercept = 0, linetype = "dashed", alpha=0.7, color = "grey") + coord_equal() + annotate("text", x = .75, y = .15, label = paste("CAD4TBv6: ", round(roc_CAD6[2],4), " (95% CI:", round(roc_CAD6[1],4), "-", round(roc_CAD6[3],4), ")", ", ", "\n",
                       "CAD4TBv7: ", round(roc_CAD7[2],4), " (95% CI:", round(roc_CAD7[1],4), "-", round(roc_CAD7[3],4), ")", sep = ""), size = 3) +
labs(x = "1-Specificity", y= "Sensitivity", subtitle = paste0("The ROC curves of  CAD4TB using Xpert results as the reference (n=", length(MDF$PID_OMRS), ")"))+theme(legend.position = "bottom")



tiff("2.0 Version Comparison/Figure-2 ROCs.tif", width = 12, height = 8, units = "in", res = 100)
require(gridExtra)
