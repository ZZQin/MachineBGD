# library(tableone)
# 
# MDF[MDF$Xpert2Outcome_num %in% "1" & MDF$qXRv3_100 <0, c(1, 3, 7, 9, 14:19, 26:29)]
# 
# LowQXR <- MDF[MDF$Xpert2Outcome_num %in% "1" & MDF$qXRv3_100 <45, c(1, 3, 7, 9, 14:19, 26:29)]
...
# HighJF <- MDF[MDF$Xpert2Outcome_num %in% "0" & MDF$qXRv3_100 >80, c(1, 3, 7, 9, 14:19, 26:29)]
# 
# 
# ## High JF score, false positive ------------------
# MDF$HighJF <- "Normal"
# MDF$HighJF[MDF$Xpert2Outcome_num %in% "0" & MDF$qXRv3_100 >80] <- "FP"
# 
# # Define numeric variables
# listVar <- c("Age", "Gender",  "Xpert2Outcome_num", "MTB.Burden", "RIF.Result", "Radiology.Result", "rad.highly.TB", "rad.TB", "rad.abn", "CAD4TB6", "qXRv3", "LunitScore", "JF1", "HighJF")
# 
...
# MDF$qXR_C <- cut(MDF$qXRv3_100, breaks = 50)
...
cutoff <- CAD_Xpert_plot$Score[which(abs(CAD_Xpert_plot$Sens -0.95)==min(abs(CAD_Xpert_plot$Sens -0.95)))]
length(MDF[MDF$qXRv3 < cutoff & MDF$Xpert2Outcome_num %in% "1", 2])
MDF$qXRMissed[MDF$qXRv3 < cutoff & MDF$Xpert2Outcome_num %in% "1"] <- "Missed by qXR"
...
MDF$Comment[MDF$Radiology.Result == "X-Ray Normal" & MDF$count==0 & MDF$Xpert2Outcome_num %in% "1"] <- "Missed by human ONLY"

MDF$Comment[MDF$Radiology.Result != "X-Ray Normal" & MDF$count==1 & MDF$Xpert2Outcome_num %in% "1"] <- "Missed by 1 AI but NOT human"
MDF$Comment[MDF$Radiology.Result == "X-Ray Normal" & MDF$count==1 & MDF$Xpert2Outcome_num %in% "1"] <- "Missed by 1 AI AND by human"


MDF$Comment[MDF$Radiology.Result != "X-Ray Normal" & MDF$count==2 & MDF$Xpert2Outcome_num %in% "1"] <- "Missed by 2 AI but NOT human "
MDF$Comment[MDF$Radiology.Result == "X-Ray Normal" & MDF$count==2 & MDF$Xpert2Outcome_num %in% "1"] <- "Missed by 2 AI AND human "

MDF$Comment[MDF$Radiology.Result != "X-Ray Normal" & MDF$count==3 & MDF$Xpert2Outcome_num %in% "1"] <- "Missed by 3 AI but NOT human "
MDF$Comment[MDF$Radiology.Result == "X-Ray Normal" & MDF$count==3 & MDF$Xpert2Outcome_num %in% "1"] <- "Missed by 3 AI AND human "

MDF$Comment[MDF$Radiology.Result != "X-Ray Normal" & MDF$count==4 & MDF$Xpert2Outcome_num %in% "1"] <- "Missed by 4 AI but NOT human "
MDF$Comment[MDF$Radiology.Result == "X-Ray Normal" & MDF$count==4 & MDF$Xpert2Outcome_num %in% "1"] <- "Missed by 4 AI AND human "

MDF$Comment[MDF$Radiology.Result != "X-Ray Normal" & is.na(MDF$CADMissed) == F & is.na(MDF$IF2Missed) == F & is.na(MDF$JF1Missed) == F & is.na(MDF$LunitMissed) == F & is.na(MDF$qXRMissed) == F & MDF$Xpert2Outcome_num %in% "1"] <- "Missed by 5 AI but NOT human"

MDF$Comment[MDF$Radiology.Result == "X-Ray Normal" & MDF$count==5 & MDF$Xpert2Outcome_num %in% "1"] <- "Missed by 5 AI AND human "

table(MDF$Comment)
# View(MDF[, c(1, 3, 13, 46:52)])



MDF$Comment2 <- ""
MDF$Comment2[MDF$JF1>=0.9 & MDF$Xpert2Outcome_num %in% "1" & MDF$TB.Medication.History %in% "No"] <- "Bac+ &high JF  New (0.5%)"
MDF$Comment2[MDF$JF1>=0.9 & MDF$Xpert2Outcome_num %in% "1" & MDF$TB.Medication.History %in% "Yes"] <- "Bac+ &high JF WITH TB History (1%)"

MDF$Comment2[MDF$Radiology.Result == "X-Ray Normal" & MDF$Xpert2Outcome_num %in% "0"] <- "Bac- human normal" 
...
# View(MDF[, c(1, 3, 7, 26:29, 32, 44:51)])


##### Additional Read June -------------------------------------

Teleradiology <- read.csv("Teleradiology.csv")
str(Teleradiology)
