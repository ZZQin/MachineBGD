...
### ROC
base <- ggplot(SubgroupPlot, aes(X, Sens)) + geom_path(aes(color = subgroup)) 
...
### Save
tiff(paste0("Results/6 Curves (no CI) of ", unique(SubgroupPlot$DeepLearningSystem, "in subgroups.tif")), width = 15, height = 10, units = "in", res = 100)
require(gridExtra)
